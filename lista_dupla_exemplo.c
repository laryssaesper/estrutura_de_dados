...
no *crialde(int n){
	no *ini, *p, *aux; //ponteiro para início, ponteiro e auxiliar
	int valor;
	ini=NULL;
	for(int i=0; i<n; i++){
		...
		if(ini != NULL){
			...
			aux=p; //na criação da lista, a aux fica parada na cabeça, para poder retornar a lista inteira
		}
		
		p->dlink=ini;
		ini=p;
	}
	return aux; //a função é um ponteiro, pois retorna um ponteiro
